        ...
        <v-form ref="newMeta" v-model="valid" :key="form._id">
          <v-container fluid class="pa-0">
            <v-layout row wrap>
              ...
              <v-flex xs2 sm1>
                <div class="caption grey--text">Icon</div>
                <div class="caption grey--text"><v-icon>{{ form.mdi }}</v-icon></div>
              </v-flex>
              <v-flex xs10 sm6>
                <v-text-field
                  v-model="form.mdi"
...
export default {
  ...
  methods: {
    ...
    onLeave () {
      ...
      this.form.provider = this.item.provider,
      this.form.mdi = this.item.mdi,
      this.isEditing = false;
    },
    onDelete () {
